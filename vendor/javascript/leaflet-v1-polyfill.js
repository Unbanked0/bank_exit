...
function applyBrowserPolyfill() {
  ...
  L.Browser.svg = !!(
    document.createElementNS && L.SVG.create("svg").createSVGRect
  );
  L.Browser.inlineSvg =
    !!L.Browser.svg &&
    (function () {
  ...
  L.Browser.any3d =
    !window.L_DISABLE_3D && (L.Browser.webkit3d || L.Browser.gecko3d);
  L.Browser.vml = false;
...
function applyDomUtilPolyfill() {
  ...
  L.DomUtil.setClass = (el, name) => {
    el.classList.value = name;
  };
...
function applyUtilPolyfill() {
  ...
  L.Util.getParamString = function (obj, existingUrl, uppercase) {
    ...
    return (
      (!existingUrl || !existingUrl.includes("?") ? "?" : "&") +
      params.join("&")
    );
  };
    ...
    _findEventTargets(e, type) {
      ...
      for (let i = 0; i < targets.length; i++) {
        if (targets[i].options.bubblingMouseEvents !== undefined) {
          targets[i].options.bubblingPointerEvents =
...
function applyDomEventPolyfill() {
  ...
  L.DomEvent.getWheelDelta = function (e) {
    return e.deltaY && e.deltaMode === 0
      ? -e.deltaY / L.DomEvent.getWheelPxFactor() // Pixels

...
RSpec.describe 'Merchants::Reports' do
  ...
  describe 'GET /merchants/:id/report/new' do
    context 'when merchant exists' do
      subject! { get "/merchants/#{merchant.identifier}/report/new", as: :turbo_stream }

      it { expect(response).to have_http_status :ok }
    end

    context 'when merchant does not exist' do
      subject! { get "/merchants/#{invalid_merchant_id}/report/new", as: :turbo_stream }

      it { expect(response).to have_http_status :not_found }
    end
  end

  describe 'POST /merchants/:id/report' do
    subject(:action) do
      post "/merchants/#{merchant.identifier}/report",
           params: params,
           as: :turbo_stream
    end
    ...
    context 'when params are valid' do
      ...
      context 'when Github API responds successfully' do
        before do
          stub_request(:post, /api.github.com/)
            .with(body: {
              ...
              labels: %w[merchant anomaly english]
            }.to_json)
        end
        ...
        describe '[HTTP Status]' do
          ...
          it { expect(response).to have_http_status :ok }
          it { expect(flash[:notice]).to eq(I18n.t('merchants.reports.create.notice')) }
        end
      end

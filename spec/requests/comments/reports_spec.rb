...
RSpec.describe 'Comments::Reports' do
  ...
  describe 'GET /en/comments/:comment_id/report/new' do
    context 'when merchant exists' do
      subject! { get "/en/comments/#{comment.id}/report/new", as: :turbo_stream }

      it { expect(response).to have_http_status :ok }
    end

    context 'when merchant does not exist' do
      subject! { get "/en/comments/#{invalid_comment_id}/report/new", as: :turbo_stream }

      it { expect(response).to have_http_status :not_found }
    end
  end

  describe 'POST /en/comments/:comment_id/report' do
    subject(:action) do
      post "/en/comments/#{comment.id}/report",
           params: params,
           as: :turbo_stream
    end
    ...
    context 'when params are valid' do
      ...
      describe '[HTTP Status]' do
        ...
        it { expect(response).to have_http_status :ok }
        it { expect(flash[:notice]).to eq(I18n.t('comments.reports.create.notice')) }
      end
    end
